...
## Type 빠른 참조
`feat` 기능 · `fix` 버그 · `refactor` 구조 · `docs` 문서 · `style` 포맷 · `test` 테스트 · `chore` 설정/툴링 · `prompt` Codex 프롬프트(docs/prompts.md 버전 동반)
...
- 한 커밋에 관심사 섞지 않기
- 비밀키/토큰/.env 값 커밋 금지
- Anthropic 커밋 푸터(`Co-Authored-By: Codex ...`)는 프로젝트 정책에 따라 유지
